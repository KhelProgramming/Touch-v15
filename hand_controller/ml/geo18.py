...
def extract_geo18(hand_landmarks) -> List[float]:
    ...
    palm_width = float(np.linalg.norm(p[5] - p[17]))
    if palm_width < 1e-6:
        palm_width = 1.0

    wrist = p[0]
    extensions = [float(np.linalg.norm(wrist - p[i]) / palm_width) for i in (4, 8, 12, 16, 20)]
    thumb_tip = p[4]
    pinches = [float(np.linalg.norm(thumb_tip - p[i]) / palm_width) for i in (8, 12, 16, 20)]
    spreads = [float(np.linalg.norm(p[i] - p[j]) / palm_width) for i, j in ((8, 12), (12, 16), (16, 20))]
    thumb_to_pinky_base = float(np.linalg.norm(p[4] - p[17]) / palm_width)
    angles = [
